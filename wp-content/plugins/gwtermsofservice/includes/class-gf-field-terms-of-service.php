...
class GF_Field_Terms_Of_Service extends GF_Field_Checkbox {
	...
	public function get_style_block( $form, $entry, $value ) {
		...
		$disable_css = gf_apply_filters( 'gptos_disable_css', $form['id'], false, $this, $form['id'], $entry_id, $value );
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		if( $this->is_entry_detail() ) {
			...
			if( ! $this->is_html_enabled( $this ) ) {
				...
				$terms = sprintf(
					...
					$this['id'], esc_attr( implode( ' ', $classes ) ), $terms
				);
				...
				$terms = sprintf(
					...
					"</div>",
					$this->id, esc_attr( implode( ' ', array_filter( $classes ) ) ), $tabindex, $terms
				);
		...
		if( ! is_admin() || GP_Perk::doing_ajax( 'gpnf_edit_entry' ) ) {
		...
		$input = sprintf(
			...
			$html_id, $this->get_checkbox_choices( $value, $disabled, $form['id'] )
		);
